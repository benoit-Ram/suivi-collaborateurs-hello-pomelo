...
export class MissionsService {
  ...
  async findAll(filters?: Record<string, string>) {
    let query = this.supabase.db.from('missions').select('*, assignments(*, collaborateurs:collaborateur_id(id, prenom, nom, photo_url, poste))').order('created_at', { ascending: false });
    if (filters) {
  ...
  async findOne(id: string) {
    const { data, error } = await this.supabase.db.from('missions').select('*, assignments(*, collaborateurs:collaborateur_id(id, prenom, nom, photo_url, poste, equipe))').eq('id', id).single();
